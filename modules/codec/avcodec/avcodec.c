...
static int OpenDecoder( vlc_object_t *p_this )
{
    ...
    msg_Info( p_dec, "######## Need to decode this codec: (%s)", psz_namecodec );
    ...
    msg_Info( p_dec, "######## Ask avcodec to get (%s) decoder", psz_decoder );
    ...
    if( psz_decoder && *psz_decoder )
    {
        ...
        if( !p_codec )
        {
            msg_Info( p_dec, "######## Decoder `%s' not found", psz_decoder );
            msg_Err( p_this, "Decoder `%s' not found", psz_decoder );
        }
        else if( p_codec->id != i_codec_id )
        {
            msg_Info( p_dec, "######## Decoder `%s' found can't handle %s", psz_decoder, psz_namecodec );
    ...
    if( !p_codec )
    {
        msg_Info( p_dec, "######## Try to find a decoder for %u codec id", i_codec_id );
        p_codec = avcodec_find_decoder( i_codec_id );
    }
    if( !p_codec )
    {
        msg_Info( p_dec, "######## Decoder not found for %s codec", psz_namecodec );
        return VLC_EGENERIC;
    }
