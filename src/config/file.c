...
int config_CreateDir(vlc_object_t *p_this, const char *psz_dirname)
{
	...
	case ENOENT:
	{
		...
		//vz  char psz_parent[strlen( psz_dirname ) + 1], *psz_end;
		char  *psz_end;
		char*  psz_parent = NULL;
		const int size = strlen(psz_dirname) + 1;
		psz_parent = malloc(sizeof(char)*size);
